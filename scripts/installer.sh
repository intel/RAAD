...
# Parameters
EXE_CANDIDATE=src/installer.py
MACID=com.intel.labs.radengine

SAVEPATH=data/installer
...
pyinstaller --windowed --noconfirm --log-level=DEBUG --clean --onefile --key=$KEY --workpath=$TMPPATH --distpath=$SAVEPATH --specpath=$SAVEPATH --name=$FILENAME $EXE_CANDIDATE 2>&1 | sed -r 's/'$(echo -e "\033")'\[[0-9]{1,2}(;([0-9]{1,2})?)?[mK]//g' | tee $LOGFILE

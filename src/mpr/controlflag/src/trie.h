...
// expression, size_t for distance, size_t for num_occurance.
struct NearestExpression {
 public:
    using Expression = std::string;
    using Cost = size_t;
    using NumOccurances = size_t;

 public:
    NearestExpression() : expression(""), cost(0), num_occurances(0) {}
    NearestExpression(const Expression& expression, Cost cost,
                      NumOccurances occurances=1) : expression(expression),
                      cost(cost), num_occurances(occurances) {}
    ...
    NumOccurances GetNumOccurances() const  { return num_occurances; }
 ...
 private:
    ...
    Cost cost;
    /// Number of occurances of this expression in training dataset
    NumOccurances num_occurances;
};
...
class Trie {
 public:
  struct TrieNode {
   public:
    TrieNode(char c, size_t num_occurances=0, bool terminal_node=false,
              float confidence=0) : c_(c), num_occurances_(num_occurances),
  ...
  void Build(const std::string& train_dataset) {
    ...
    auto is_ast_expression = [&](const std::string& line, size_t& comma_pos) -> bool {
    ...
    while (std::getline(stream, line)) {
      ...
      if (is_ast_expression(line, comma_pos)) {
        ...
        size_t contributor_id = std::strtoul(contributor_id_str.c_str(), NULL, 10);
  ...
  // number of occurances (ranking criteria).
 ...
 private:
  ...
  inline void GenerateListOfAllTriePaths() {
    ...
    if (all_trie_paths.size() == 0) {
      VisitAllLeafNodes([&](const std::string& trie_path, size_t num_occurances,
          PatternContributorsMap pattern_contributors) {
        all_trie_paths.push_back(std::make_pair(trie_path, num_occurances));
      });
    }
  }

#ifndef TREE_ABSTRACTION_H
#define TREE_ABSTRACTION_H
...
inline std::string NodeToShortString(const TSNode& conditional_expression) {
  std::string expression_string = NodeToString<L>(conditional_expression);
...
inline std::string AbstractTerminalString(const TSNode& node) {
  std::string ret;
#if 0
  std::vector<std::string> allowed_literals = {"identifier",
    "number_literal"};

  bool found = false;
  for (auto& literal : allowed_literals) {
    if (literal.compare(ts_node_type(node)) == 0) {
      ret += literal;
      found = true;
      break;
    }
...
inline std::string AbstractBinaryExpressionString(const TSNode&
  ...
  //ret += "left: (" + AbstractTerminalString(lhs) + ") ";
  ret += "(" + AbstractTerminalString(lhs) + ") ";
  //ret += "right: (" + AbstractTerminalString(rhs) + ")";
...
inline std::string AbstractUnaryExpressionString(
  ...
  cf_assert(ts_node_named_child_count(unary_expression) ==
         num_comment_nodes + 1, "Unexpected unary expression:", unary_expression);

  //ret += "argument: " + AbstractTerminalString(arg);
...
inline std::string AbstractSubscriptExpressionString(
  ...
  int num_comment_nodes = 0;
  for (uint32_t i = 0; i < ts_node_named_child_count(subscript_expression); i++) {
  ...
  //ret += "argument: (" + AbstractTerminalString(arg1) + ") ";
  ret += "(" + AbstractTerminalString(arg1) + ") ";
  //ret += "index: (" + AbstractTerminalString(arg2) + ")";
...
inline std::string OpToString(const TSNode& node) {
  ...
  std::string orig_op_string = ts_node_string(op);
...
inline std::string AbstractParenthesizedExpressionString(const TSNode&
  ...
  if (parenthesized_expression.compare(ts_node_type(node)) == 0) {
    ...
    ret += "(field_expression argument: (identifier) field: (field_identifier))";
...
template <>
inline std::string NodeToString<LEVEL_TWO>(
    const TSNode& conditional_expression) {
  std::string ret;
...
template <>
inline std::string NodeToString<LEVEL_ONE>(const TSNode& node) {
  std::string ret = "";
  ...
  for (uint32_t i = 0; i < children; i++) {
    ...
    ret += NodeToString<LEVEL_ONE>(child);
  }
...
template <>
inline std::string NodeToString<LEVEL_MIN>(
    const TSNode& conditional_expression) {

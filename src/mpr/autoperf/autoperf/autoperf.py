...
from tensorflow.python.keras import models as TFKModel
from rich.progress import track

from autoperf.plots import plot_roc_curve, plot_histograms, plot_pr_curve
...
from autoperf.keras_autoencoder import trainAutoEncoder
...
def getMSE(old: np.ndarray, new: np.ndarray) -> float:
    ...
    aggregateMean = np.mean(squaredError)
    floatAggregateMean = float(aggregateMean)
    return floatAggregateMean
...
def detectAnomalyPoints(realData: np.array, predictedData: list, datasetHeader: list,
...
def testAnomaly(model: TFKModel, testDataDir: str, runs: list,
                thresholdError: float) -> tuple[list[AnomalyTuple], list[float]]:
    """Collects AnomalyTuples from test run inferences in the given directory.

    Args:
        model: A Keras autoEncoderObj.
        testDataDir: A directory holding performance data.
    ...
    for run in runs:
        ...
        datasetLen, anomalyCount, ranking = detectAnomalyPoints(dataArray, decoded_data_list, datasetHeader,
                                                                thresholdError)
...
{pct:.2f}% - {summary.ranking[:3]}', file=logFile)
    ...
    if len(topRankings) > 0:
        ...
        if desc is not None:
            logMessage = f'- Explanation: [{desc}]'
        else:
            logMessage = f'- Performance counter not found in {getAutoperfDir("descriptions.json")}.'
        print(logMessage, file=logFile)
    return


def testModel(model: TFKModel, thresholdError: float, nonAnomalousDataDir: str,
    ...
    plot_roc_curve(error_labels, combined_errors)
    plot_pr_curve(error_labels, combined_errors)
...
def getReconstructionErrors(perfTestDataDir: str, model: TFKModel) -> list:
...
def aggregateAndTrain(perfTrainDataDir: str, autoencoder: TFKModel, saveTrainedNetwork: bool = False,
                      outputDir: str = None) -> TFKModel:
    ...
    Args:
        ...
        autoencoder: A Keras autoEncoderObj.
    ...
    Returns:
        A trained Keras autoEncoderObj.
    """
    cfg = loadConfig_TryDetect()
    ...
    for train_run in training_sequence:
        ...
        pDataList = list(processed_dataset_array)
        dataset.extend(pDataList)
    ...
    model, history = trainAutoEncoder(autoEncoderObj=autoencoder, trainingData=trainingDataset, monitor=monitor)
    rHistory = history['record']
    aHistory = rHistory['batch_end']['loss']
    train_loss_list.extend(aHistory)
    validation_loss_list.extend(aHistory)
...
def calcThresholdError(reconstructionErrors: list) -> float:
    ...
    floatMeanVal = float(meanVal)
    return floatMeanVal


def trainAndEvaluate(model: TFKModel, trainDataDir: str):
    """Trains and performs some basic evaluation on an autoEncoderObj.
    ...
    thresholdError = calcThresholdError(datasetTrainErrors)
    np.save(getAutoperfDir('threshold.npy'), np.array([thresholdError]))

    plot_histograms(train=np.array(datasetTrainErrors),

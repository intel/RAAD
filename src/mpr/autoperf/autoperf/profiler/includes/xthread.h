...
public:
  ...
  void initialize() {
    ...
    // Allocate the thread index for current thread
    initInitialThread();
  }
  ...
  int allocThreadIndex(void) {
    ...
    if (index >= xdefines::MAX_THREADS || alivethreads >= xdefines::MAX_ALIVE_THREADS) {
      fprintf(stderr,
              "Set xdefines::MAX_THREADS to larger. _alivethreads %ld totalthreads %ld maximum alive threads %d",
              _aliveThreads, _threadIndex, xdefines::MAX_ALIVE_THREADS);
      abort();
    }
    ...
    if (alivethreads == 0) {
      ...
      // Now we need to get elapse of the serial phase
      stopThreadLevelInfo();

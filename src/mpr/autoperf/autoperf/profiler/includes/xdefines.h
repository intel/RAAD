...
typedef struct {
  int mark; // unique identified of code location
  // long long count[NUM_EVENTS];
  long long count[NUM_EVENTS_MAGIC];
} perf_record_t;
...
public:
  ...
  enum {
    INTERNALHEAP_SIZE = 1048576UL * 1024 * 8
  };
  enum {
    PAGE_SIZE = 4096UL
  };
  ...
  enum {
    MAX_PERF_RECORDS_PER_THREAD = 1000000
  };
  enum {
    NUM_EVENTS_TO_MONITOR = 2
  };
  ...
  enum {
    MAX_ALIVE_THREADS = NUM_HEAPS
  };
  enum {
    MAX_THREAD_LEVELS = 256
  };

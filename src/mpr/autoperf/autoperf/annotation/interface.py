...
class DirectoryTree():
    ...
    def __init__(self, paths, top_dir):
        self.root = Tree(f":open_file_folder: [link file://{pathlib.Path('.').resolve()}]{top_dir}/",
    ...
    def add_file(self, path: str):
        ...
        for i, subd in enumerate(list_of_subdirs):
            ...
            if subd not in self.dirs:
                ...
                self.dirs[subd] = self.dirs[prev_subd].add(f":open_file_folder: [link file://{pathlib.Path(subd).resolve()}]{final_dir}/",
                                                           guide_style="bright_blue")

        # Then, add the file as a leaf node
        text_filename = Text(pathlib.Path(path).name, "bold bright_black")
        text_filename.stylize(f"link file://{pathlib.Path(path).resolve()}")
        icon = "📄 "
        self.files[path] = self.dirs[str(pathlib.Path(path).parent)].add(Text(icon) + text_filename)

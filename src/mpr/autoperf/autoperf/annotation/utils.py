...
def get_perf_include() -> list:
    ...
    return safe_format(perf_include, banner=banner).splitlines(True)
...
def get_perf_start(mark_id: int) -> list:
    ...
    return safe_format(perf_start, banner=banner, mark_id=mark_id).splitlines(True)


def get_perf_end() -> list:
    """Get the `perfpoint_END()` banner."""
    # return perf_end.format(banner=banner).splitlines(True)
    return safe_format(perf_end, banner=banner).splitlines(True)
    ...
    def inject(self, result: ParseResult):
        ...
        for _, func in functions:
            ...
            if banner in lines[start + 1]:
                ...
                strContext = safe_format(already_annotated, func=func['spelling'], file=result.filename)
                print_queue.append(strContext)
                continue
            strContextSpelling = safe_format(annotating, func=func['spelling'], file=result.filename)
            print_queue.append(strContextSpelling)

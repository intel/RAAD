...
extensions = ['c++', '.cpp', '.hpp', '.cxx', '.hxx', '.c', '.h', '.cc', '.hh', 'inl']
...
def get_parser_options(fast: bool = False, preproc: bool = False) -> int:
    ...
    default_parser_options = (
                CXTranslationUnit_VisitImplicitAttributes  # only way to get class methods parsed correctly
...
class ParseResult():
    ...
    def get_table(self) -> Table:
        ...
        """
        import pathlib
        resolvedName = pathlib.Path(self.filename).resolve()

        title = Text(f'{self.filename}', style=Style(italic=True, bold=True, link=f'file://{resolvedName}'))
        ...
        if len(self.d.items()) == 0:
            ...
            fSuffix = pathlib.Path(self.filename).suffix
            if fSuffix in extensions:
...
class ClangParser():
    ...
    def get_args_from_makefile(self, root: pathlib.Path = pathlib.Path('.'), attempt: int = 0):
        ...
        from autoperf.utils import prependMake
        prependMake(fileLocation=mkfile)
        try:
            with open(mkfile) as f:
                # stageString = makefile_found.format(mkfile=mkfile)
                from autoperf.utils import safe_format
                stageString = safe_format(makefile_found, mkfile=mkfile)
                log.info(stageString)
                for item in f.read().split('\n'):
                    # Remove 'CFLAGS =', any comments, and strip whitespace
                    if item.startswith('CFLAGS'):
                        args = item.split('=')[1].split('#')[0].strip()
                        # stageArgs = makefile_args.format(args=args)
                        stageArgs = safe_format(makefile_args, args=args)
                        log.info(stageArgs)
                        self.args = args
                        return

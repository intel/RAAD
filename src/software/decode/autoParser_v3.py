...
# *****************************************************************************/
# Extract compiled code into interpreted code
################################################################################################################
# General Python module imports
################################################################################################################
import os, re, sys, logging, platform, json, copy, pprint, datetime, pickle

from src.software.decode.configs import dataStructure
...
# from __builtin__ import classmethod, int, long  # @todo cleanup explicit usage
# from exceptions import *  # @todo cleanup explicit usage
GHS_COMPILER_VERSION = "comp_201754"
...
def pressReturnToContinue(aString=None, indent=0, enable=ENABLE_DEBUG_ENTER):
    if enable:
        if sys.version_info[0] < 3:
            if aString is None:
                usersInput = input("%sPRESS RETURN TO CONINTUE or 'q' to quit: " % (indent * ' '))
            else:
                usersInput = input("%sPRESS RETURN TO CONINTUE or 'q' to quit (%s): " % (indent * ' ', aString))
        else:
            if aString is None:
                usersInput = eval(input("{}PRESS RETURN TO CONINTUE or 'q' to quit: ".format((indent * ' '))))
            else:
                usersInput = eval(input("%sPRESS RETURN TO CONINTUE or 'q' to quit (%s): " % (indent * ' ', aString)))
        if usersInput == 'q':
            sys.exit(0)
...
#        pressReturnToContinue("setAlternateBitfieldCtypeClass Entry", enable=1)
    ...
    while re.match('^(.+\D)(\d+)$', curCtypeClass):
        ...
        if bitfieldOnlyObject:
            if (obj.size > int(m.group(2)) / 2) or (nextCtypeClass not in list(BASE_CTYPE_SIZE_BYTES.keys())): break
        elif (bitCount > int(m.group(2)) / 2) or (nextCtypeClass not in list(BASE_CTYPE_SIZE_BYTES.keys())): break
        curCtypeClass = nextCtypeClass

    # if BASE_CTYPE_SIZE_BYTES[curCtypeClass] * 8 >= bitCount:
    #     for idx in xrange(startIdx, endIdx + 1):
...
#            pressReturnToContinue('pliActiveBandNum',enable=1)
        ...
        elif m.typedef in list(BASE_CTYPE_MAP.keys()) and BASE_CTYPE_SIZE_BYTES[BASE_CTYPE_MAP[m.typedef]] >= maxSize:
        ...
        if (self.dataType is not None) and (self.dataType.lower() == 'enum'):     # Generate hash for enumeration type
            ...
            if self.typedef not in generatedCtypeClasses:
                ...
                for m in self.memberList:
                    ...
                    elif m.typedef in list(BASE_CTYPE_MAP.keys()):
                        ctypeClass = BASE_CTYPE_MAP[m.typedef]
                    elif m.dataType in list(BASE_CTYPE_MAP.keys()):
                        ctypeClass = BASE_CTYPE_MAP[m.dataType]
                    else: ctypeClass = m.typedef

                    # Renaming to avoid using Python keywords as variable names
                    if m.instance in list(VARIABLE_KEYWORD_REMAP.keys()):
...
#           unionClassString += "        self.buffer = cast(self.libc.calloc(1, self.bufferSize), POINTER(c_ubyte))\n"
            ...
            unionClassString += "            readFile(inFile, buffer=self.Data, numBytes=self.ctypeSize()/8)\n"
            ...
            if self.versionMajorMemberName is not None:
                ...
                unionClassString += "            except Exception as e: return None\n"
            ...
            utilityMethodsString += "            buffer[index] = ord(b)\n"
            utilityMethodsString += "        except TypeError:\n"
            utilityMethodsString += "            buffer[index] = b\n"
...
class AutoParser(object):
    ...
    Attributes:
        ...
        in multiple stages.
    """
    def __init__(self, options=None, isAPI=False):
        self.isAPI = isAPI
        try:
            if platform.system() == 'Linux':
                self.ghsPath = '/usr/ghs'
            elif 'CYGWIN_NT' in platform.system():
                self.ghsPath = 'c:/ghs'
            elif platform.system() == 'Windows':
                self.ghsPath = 'c:/ghs'
                ...
                self.ghsPath = os.getcwd()
        except Exception:
            print("Failed binary exe")
        if (isAPI is False) and (options is not None):
            self.__setupOptions_Default(options=options)
        else:
            print("Selecting Python API...", flush=True)

    def __setupOptions_Default(self, options):
        ...
        # self.ghsPath = ghsPath # @todo within the init...

        self.recursive = False
        self.subStructList = set()
        self.masterObjectList = {}  # Key=UID: [Object, Struct, DG]
        ...
        try:
            ...
            if self.options.inputCfg is not None:
                ...
                self.AUTOPARSER_DATA_OBJECT_LIST = dataStructure.AUTOPARSER_DATA_OBJECT_LIST
        except Exception:
            print("Error in importing CFG file:  {}".format(self.options.inputCfg), flush=True)
            self.AUTOPARSER_DATA_OBJECT_LIST = BUILT_IN_DATA_OBJECT_LIST
            pass

        return

    def __setupOptions_API(self,
                       fwBuildOutputDir=None,
                       projectName=None,
                       ghsCompilerVersion=None,
                       inputCfg=None,
                       autoParseList=None,
                       gdumpExe="autoParserAnalyzer.exe",
                       versionedParserFolder='versioned',
                       unversionedParserFolder='unversioned',
                       jsonParserFolder='json',
                       cFormatHeaderFolder='include',
                       directoryPrefix='autoParser',
        ...
        ######################################################################
        ######################################################################
        outDir = os.path.abspath(os.path.join(fwBuildOutputDir, directoryPrefix))
        if not os.path.exists(outDir):
            try:
                ...
                if debug:
                    quit(2)
        print("self.outDir:                 {}".format(outDir))

        currentTimeUTCString = datetime.datetime.utcnow().strftime(timeFormat)
        autoParseFile = "{}_{}.log".format(directoryPrefix, currentTimeUTCString)
        logFileName = os.path.join(self.outDir, autoParseFile)

        # '%m/%d/%Y %I:%M:%S %p'
        logging.basicConfig(filename=logFileName, filemode='w', format='%(asctime)s %(levelname)s:%(message)s', datefmt=timeFormat, level=logging.DEBUG)
        logging.info('{} Log'.format(__file__))

        dbgDumpFile = os.path.join(outDir, '{}_dbgDump.txt'.format(directoryPrefix))

        if autoParseList is not None and inputCfg is None:
            # Validate objects
            validCount = 0
            for (dataIter, dataItem) in enumerate(autoParseList):
                itemCount = 0
                for dataIterRow, dataItemRow in enumerate(dataItem):
                    if (dataItemRow is not None):
                        itemCount += 1
                if itemCount > 3:
                    validCount += 1
            if validCount is len(autoParseList):
                AUTOPARSER_DATA_OBJECT_LIST = autoParseList
            else:
                AUTOPARSER_DATA_OBJECT_LIST = BUILT_IN_DATA_OBJECT_LIST
        elif autoParseList is None and inputCfg is not None:
            # If there is an input CFG file to guide parser lib generation, use it instead.
            inputCgfFile = os.path.abspath(inputCfg)
            print("Input CFG: %s" % inputCgfFile)
            try:
                exec(open(inputCgfFile).read())
                if not dataStructure.AUTOPARSER_DATA_OBJECT_LIST:  # @todo After the exec() command, there should be a list named AUTOPARSER_DATA_OBJECT_LIST.
                    print("\nProblem importing the specified input CFG file (%s).\n" % inputCgfFile)
                    sys.exit(0)
            except Exception:
                print("\nProblem importing the specified input CFG file (%s).\n" % inputCgfFile)
        ...
        # Data members needed for Python C-Type generation
        ######################################################################
        versionedParserFolder = os.path.join(outDir, versionedParserFolder)
        if not os.path.exists(versionedParserFolder):
            try:
                os.makedirs(versionedParserFolder)
            except OSError:
                print("Failed to create the versioned parser folder")
                if debug:
                    quit(7)

        unversionedParserFolder = os.path.join(outDir, unversionedParserFolder)
        if not os.path.exists(unversionedParserFolder):
            try:
                os.makedirs(unversionedParserFolder)
            except OSError:
                print("Failed to create the unversioned parser folder")
                if debug:
                    quit(7)

        jsonParserFolder = os.path.join(outDir, jsonParserFolder)
        if not os.path.exists(jsonParserFolder):
            try:
                os.makedirs(jsonParserFolder)
            except OSError:
                print("Failed to create the json parser folder")
                if debug:
                    quit(7)

        cFormatHeaderFolder = os.path.join(outDir, cFormatHeaderFolder)
        if not os.path.exists(cFormatHeaderFolder):
            try:
                os.makedirs(cFormatHeaderFolder)
            except OSError:
                print("Failed to create the C-format header folder")
                if debug:
                    quit(7)

        self.__doDebugDataDump(gdumpExe)
        ####################
        if not os.path.exists(dbgDumpFile):
            print("\nSomething is wrong.  {} does not exist!!!".format(self.dbgDumpFile))
            sys.exit(0)
        ...
        self.dbgDumpFile = dbgDumpFile
        # self.ghsPath = ghsPath # @todo within the init...
        self.versionedParserFolder = versionedParserFolder
        self.unversionedParserFolder = unversionedParserFolder
        self.jsonParserFolder = jsonParserFolder
        self.cFormatHeaderFolder = cFormatHeaderFolder
        self.gdumpExe = gdumpExe
        self.AUTOPARSER_DATA_OBJECT_LIST = AUTOPARSER_DATA_OBJECT_LIST
        self.options = None  # API does not have options...
    ...
    def __buildCtypeParser(self, obj, dbgData):
        ...
        while idx < len(dbgData):
            ...
            # 8:       "uint64_t" Typedef  Info C ULongLong
                ...
                if obj.typedef in list(BASE_CDATATYPE_SIZE_MAP.keys()) and (obj.size is None or obj.size == 0):
                    obj.size = BASE_CDATATYPE_SIZE_MAP[obj.typedef]*8
                elif obj.dataType in list(BASE_CDATATYPE_SIZE_MAP.keys()) and (obj.size is None or obj.size == 0):
    ...
    def __generateCtypeParserFile(self, obj, outFile=None, generatedCtypeClasses=[], versioned=True):
        ...
        if outFile is None:
            ...
            with open(outParserFile, "wb") as pyParserFile:
                ...
                autoParserTxt += "    xrange  # @todo python 2 convert\n"
                autoParserTxt += "except NameError:\n"
                autoParserTxt += "    # Python 3\n"
                autoParserTxt += "    xrange = range\n"
                autoParserTxt += "\n"
    ...
    def __createInitFiles(self):
        ...
        outputString += "    xrange  # @todo python 2 convert\n"
        outputString += "except NameError:\n"
        outputString += "    # Python 3\n"
        outputString += "    xrange = range\n"
        outputString += "\n"
        ...
        outputString += "    outString += \"    xrange  # @todo python 2 convert\\n\"\n"
        outputString += "    outString += \"except NameError:\\n\"\n"
        outputString += "    outString += \"    # Python 3\\n\"\n"
        outputString += "    outString += \"    xrange = range\\n\"\n"
        outputString += "    outString += \"\\n\"\n"
        ...
        outputString += "    outString += \"        except Exception as e:\\n\"\n"
...
def main():
    ...
    if (options.fwBuildOutputDir is None) or (options.projectName is None):
    ...
    humanOptions = pprint.pformat(options)
    print("Friendly View of Options Selected\n {}\n".format(humanOptions))
